...
def switch_append(question_list, switch_answer, messages_list, my_chat, timeout_check):
    if len(question_list) > 1 and question_list[-2] not in ["gpt-3", "gpt-4", "gpt3", "gpt4", "gpt-4-t", "gpt4t", "less", "vim", "cache", "config"] and switch_answer != None and len(messages_list) != 1 and timeout_check == True:
